...
<div class="fixed inset-0 bg-black/30 z-[200] flex items-center justify-center" onkeydown={(e) => e.key === 'Escape' && onclose()}>
	...
	<!-- svelte-ignore a11y_click_events_have_key_events -->
	<div class="bg-white rounded-xl shadow-2xl w-[400px] p-6" onclick={(e) => e.stopPropagation()}>
		<h3 class="text-[14px] font-semibold text-[var(--color-text-primary)] mb-4">Insert Hyperlink</h3>
		<div class="flex flex-col gap-3">
			<div>
				<label class="text-[11px] font-medium text-[var(--color-text-secondary)] mb-1 block" for="link-url">URL</label>
				<input
					id="link-url"
					type="url"
					bind:value={url}
